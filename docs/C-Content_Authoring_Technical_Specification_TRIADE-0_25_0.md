...
**Version:** 0.25.0  
**Aligned to Triade:** v0.25.0  
**Date:** 11 August 2026  
**Status:** Stage 2 authoring-schema baseline and R7 complete; **M10 dependency layer adopted**; five equipment revisions and physical child records populated and proved in an offline candidate  
...
| Area | State |
| --- | --- |
| Repository | `triade-data` created; Git initialized on `main`; R7 tooling retained; M10 dependency and equipment CSV seeds, importers, regression tests and proof artifacts implemented; local runtime artefacts ignored |
| Local authoring | R7 and the M10 dependency layer are adopted into active document `3TwLJyu7fythPjAj1e1424`; a new M10 equipment candidate is produced offline and awaits operator adoption |
...
| Calculated fields | Six intended normal Formula columns verified; no Trigger Formula is required for them |
| Authoring fields in empty tables | May remain Grist **Empty Columns** until first entry; populated dependency/equipment child columns were converted to Data Columns (`isFormula = 0`) and verified; `equipment_revision_id` remains the normal Formula column |
| `ref_rules` | **R7 complete on the supplied copy:** 143 generated rows; 143 unique IDs; 81 Critical / 48 High / 14 Medium; deterministic seed/export hash proved |
...
| ◈M10 data | **Equipment slice complete in candidate:** 5 equipment revisions, 20 `en-GB` text rows, 9 damage rows, 9 occupancy rows, and 1 Standard Shield defence row; tag rows remain empty because protected `ref_tags` is empty |
...
> **Re-proofed and implemented against 0.25.0 — 11 Aug 2026.** The dependency candidate was adopted by the operator. M·2A.7 now owns Medium's second, shallower barycentre well; the Grist Choice enum has the corresponding fourth value `barycentre_well`, and the Standard Shield construction record uses it.
>
> **Current verification boundary.** *Mechanically executed on a fresh dependency-complete copy:* five revision identities, 20 text rows, 9 damage entries, 9 occupancy rows, one shield defence row, stable-ID Reference resolution, byte-identical six-table seed/export, idempotence, unresolved-reference rejection, pip budgets, null defence-rating preservation, Bridge-2 metadata/value migration, R7 regression and SQLite integrity. *Not yet executed:* adoption of this equipment candidate into the active document, faculty/fixture population, canonical JSON, DuckDB and traces.
...
11. Five M10 equipment revisions and their text/damage/occupancy/shield-defence child records populated and proved in an offline candidate.

**Next:** adopt the M10 equipment candidate into the active Grist document, then populate the M10 faculties and fixture records.
...
Only genuine calculated fields are explicit **Formula Columns**. The current six are:
...
**Current seed:** **143 generated rows on the supplied R7 working copy — 81 Critical / 48 High / 14 Medium.** The seed and deterministic round-trip export have the identical SHA-256 `64777d393800814f683a4aed711aa00cf97f565996a010a1ccb8ad267d4d624c`.

#### 4.13.1 R7 seed contract

`ref_rules` is a generated protected reference table, not an authoring surface for new rules.

- Enumerate the **143** current rows from the regenerated Validation Rules Index, then verify each row against its home source under **P-C8**. The index is the compilation surface; the source row remains authoritative.
...
- Fail the seed build on duplicate IDs, missing source rows, index/source text drift, invalid severity, or a total other than **143 — 81 / 48 / 14**.
...
**R7 population acceptance:** 143 rows; 143 unique non-empty `rule_id` values; severity split 81/48/14; every `source_reference` resolves; rerunning the generator produces a byte-identical seed export.
...
| `content_version` | **Text** | Design/content version; rows authored against the current set use `0.25.0`. |
...
4.  construction_profiles needed by ◈M10              ADOPTED; v0.25 Bridge-2 correction in candidate
5.  equipment — five ◈M10 equipment revisions         COMPLETE IN CANDIDATE
6.  damage_profile_entries                            COMPLETE IN CANDIDATE
7.  slot_occupancy                                    COMPLETE IN CANDIDATE
8.  defence_profile_entries for Standard Shield       COMPLETE IN CANDIDATE; rating null
9.  faculties                                         NEXT
10. faculty_profiles
11. fixture_builds
12. fixture_stat_weights
13. fixture_loadouts
14. fixture_encounters — scalar fields only
15. fixture_coverage
```
...
| Shield | One hand occupied; shield budget applies | Impact 2 | 2 | Controller |
...
The current validation suite contains **143 rules — 81 Critical / 48 High / 14 Medium**, counted from the rows.

**`◈P7` closed at 0.20.0.** Every rule is authored in its home document, verified against **P-C8** — the ID leads its line or its first table cell. The suite stood at 141 rules at that release; after the 0.21.0 additions, the current source-derived set is **143 of 143**.
...
> **Verification status of this section.** R7 is mechanically and operationally complete. The generated seed contains 143 unique IDs with the 81/48/14 split; seed and export are byte-identical; the Reference metadata and `INTEGER DEFAULT 0` storage are present; existing coverage remains 0→0; a synthetic populated case exports `P-C8`, not its row number; unresolved `P-C999` aborts; idempotent rerun and SQLite integrity pass. The R7 copy was adopted into active document `3TwLJyu7fythPjAj1e1424`, and the user confirms central validation.
...
| 8 | Populate dependency profiles + five ◈M10 equipment revisions | **Complete in offline candidate; active adoption pending** |
| 9 | Populate ◈M10 faculties and fixture records | **Next after equipment adoption** |
| 10 | Add Grist generated previews/validation controls | Pending |
...
| Formula behavior | Normal Formula columns for derived values; no Trigger Formula for current six |
| Empty authoring columns | May remain Empty until first manual entry |
| `ref_rules` | Generated protected 143-row seed under §4.13.1; 81/48/14 split and deterministic hash proved |
...
| Shield defence rating | The Physical group row and stable join key exist; `rating_value` remains null because no authoritative magnitude or pooled-budget scale exists |

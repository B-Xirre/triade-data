...
def validate_seed(seed: dict[str, list[dict[str, Any]]]) -> None:
    ...
    for revision_id, row in equipment.items():
        ...
        if row["content_version"] != "0.25.0":
            raise M10EquipmentError(f"{revision_id} is not aligned to 0.25.0")
    ...
    expected_profiles = {
        ...
        "martial.shield_standard": {"impact": 2},
    ...
    if (
        ...
        or defence["rating_value"] is not None
    ):
        raise M10EquipmentError("Standard Shield defence placeholder is not source-faithful")
...
def apply_equipment(
    ...
    return {
        ...
        "shield_defence_rating_remains_null": (
            after["Defence_profile_entries"][0]["rating_value"] is None
        ),
